...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a new Foundry Prompt Agent version for the UC3 billing agent."
    )
    parser.add_argument(
        "--project-endpoint",
    ...
    parser.add_argument(
        "--agent-name",
        default=os.getenv("UC3_BILLING_AGENT_NAME", "voicecall-uc3-billing"),
        help="Foundry UC3 billing agent name (env: UC3_BILLING_AGENT_NAME)",
    )
    parser.add_argument(
        "--model",
        default=(
    ...
    parser.add_argument(
        "--prompt-file",
        default=os.getenv("UC3_BILLING_PROMPT_PATH", "assets/uc3_billing_agent_prompt.txt"),
        help="Prompt file path for the billing agent instructions",
    )
    parser.add_argument(
        "--description",
        default="VoiceCall Verify UC3 billing agent",
...
def main() -> None:
    ...
    print("Foundry UC3 billing agent version created")
    print(f"agent_name={result.name}")
    print(f"agent_version={result.version}")
    print(f"status={result.status}")
    print()
    print("Set these for UC3 billing handoff:")
    print(f"FOUNDRY_PROJECT_ENDPOINT={project_endpoint}")
    print(f"UC3_BILLING_AGENT_NAME={result.name}")
    print(f"UC3_BILLING_AGENT_VERSION={result.version}")
